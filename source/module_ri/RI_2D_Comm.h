...
// judge[is] = {s0, s1}
extern std::vector<std::tuple<std::set<TA>, std::set<TA>>> get_2D_judge(const Parallel_Orbitals& pv);
...
extern inline std::map<TA, std::map<TAC, T>> comm_map2_first(const MPI_Comm& mpi_comm,
