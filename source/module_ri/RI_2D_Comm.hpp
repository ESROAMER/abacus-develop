...
auto RI_2D_Comm::split_m2D_ktoR(const K_Vectors& kv,
                                ...
                                const int nspin,
                                const bool spgsym) -> std::vector<std::map<TA, std::map<TAC, RI::Tensor<Tdata>>>>
{
        ...
        for (const TC& cell: RI_Util::get_Born_von_Karmen_cells(period))
        {
            ...
            for (const int ik: ik_list)
            {
                ...
                { // traverse kstar, ik means ik_ibz
                    ...
                    {
                        RI::Tensor<Tdata_m> mk_2D
                            = RI_Util::Vector_to_Tensor<Tdata_m>(*mks_2D[ik_full + is_k * kv.get_nkstot_full()],
                                                                 pv.get_col_size(),
                                                                 pv.get_row_size());
                        const Tdata_m frac = SPIN_multiple
                                             * RI::Global_Func::convert<Tdata_m>(std::exp(
                                                 -ModuleBase::TWO_PI * ModuleBase::IMAG_UNIT
                                                 * ((isym_kvd.second * GlobalC::ucell.G)
                                                    * (RI_Util::array3_to_Vector3(cell) * GlobalC::ucell.latvec))));
                        set_mR_2D(mk_2D * frac);
                        ++ik_full;
                    }
                }
...
// void RI_2D_Comm::add_partial_Hexx(const K_Vectors& kv,
//                                   const int ik,
//                                   const int ix,
//                                   const double alpha,
//                                   const std::vector<std::map<TA, std::map<TAC, RI::Tensor<Tdata>>>>& Hs,
...
// {
//     ModuleBase::TITLE("RI_2D_Comm", "add_partial_Hexx");
//     ModuleBase::timer::tick("RI_2D_Comm", "add_partial_Hexx");
...
//                 const TC& cell1 = Hs_tmpB.first.second;
//                 const RI::Tensor<Tdata>& H = Hs_tmpB.second;
//                 const ModuleBase::Vector3<double> R = RI_Util::array3_to_Vector3(cell1) * GlobalC::ucell.latvec;
//                 const std::complex<double> frac
//                     = alpha * std::exp(ModuleBase::TWO_PI * ModuleBase::IMAG_UNIT * (kv.kvec_c[ik] * R)) * R[ix]
//                       * GlobalC::ucell.lat0 * ModuleBase::IMAG_UNIT;
//                 for (size_t iw0_b = 0; iw0_b < H.shape[0]; ++iw0_b)
//                 {
//                     const int iwt0 = RI_2D_Comm::get_iwt(iat0, iw0_b, is0_b);
//                     if (pv.global2local_row(iwt0) < 0)
