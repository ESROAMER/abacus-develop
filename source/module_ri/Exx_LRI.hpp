...
// Exx_LRI<Tdata>::Exx_LRI(const Exx_Info::Exx_Info_RI& info_in, const
// Exx_Info::Exx_Info_Ewald& info_ewald_in)
//     : info(info_in), info_ewald(info_ewald_in)
// {
//     this->evq = std::make_shared<Ewald_Vq<Tdata>>(this->info,
//     this->info_ewald);
// }
...
void Exx_LRI<Tdata>::init(const MPI_Comm& mpi_comm_in, const K_Vectors& kv_in) {
    ...
    this->lcaos = Exx_Abfs::Construct_Orbs::change_orbs(GlobalC::ORB,
    ...
    if (this->info.files_abfs.empty())
        ...
        this->abfs = Exx_Abfs::IO::construct_abfs(abfs_same_atom,
                                                  GlobalC::ORB,
...
void Exx_LRI<Tdata>::cal_exx_ions() {
    ...
    const double Rmax = Exx_Abfs::Construct_Orbs::get_Rmax(this->lcaos);
    // std::max(3) for gamma_only, list_A2 should contain cell {-1,0,1}. In
    // the future distribute will be neighbour.
    ...
    const std::array<Tcell, Ndim> period_Cs
        = LRI_CV_Tools::cal_latvec_range<Tcell>(2, Rmax);
    const std::pair<
        std::vector<TA>,
        std::vector<std::vector<std::pair<TA, std::array<Tcell, Ndim>>>>>
        list_As_Cs
...
template <typename Tdata>
void Exx_LRI<Tdata>::cal_exx_elec(
    ...
    for (int is = 0; is < GlobalV::NSPIN; ++is) {
        if (!(GlobalV::CAL_FORCE || GlobalV::CAL_STRESS)) {
            ...
            this->exx_lri.set_Ds(Ds[is],
                                 this->info.dm_threshold,
                                 std::to_string(is));
...
double Exx_LRI<Tdata>::post_process_Eexx(const double& Eexx_in) const {
    ...
    const double SPIN_multiple
        = std::map<int, double>{{1, 2}, {2, 1}, {4, 1}}.at(
            GlobalV::NSPIN); // why?
...
// ??? energy *= SPIN_multiple.at(GlobalV::NSPIN);			// ? energy /= 2;
// /2 for Ry
}
*/
    ...
    for (int is = 0; is < GlobalV::NSPIN; ++is) {
        ...
        for (std::size_t idim = 0; idim < Ndim; ++idim)
            for (const auto& force_item: this->exx_lri.force[idim])
                this->force_exx(force_item.first, idim)
                    += std::real(force_item.second);
    }
    ...
    for (int is = 0; is < GlobalV::NSPIN; ++is) {
        ...
        for (std::size_t idim0 = 0; idim0 < Ndim; ++idim0)
            for (std::size_t idim1 = 0; idim1 < Ndim; ++idim1)
                this->stress_exx(idim0, idim1)
                    += std::real(this->exx_lri.stress(idim0, idim1));
    }

    const double SPIN_multiple
        = std::map<int, double>{{1, 2}, {2, 1}, {4, 1}}.at(
            GlobalV::NSPIN); // why?
    const double frac = 2 * SPIN_multiple / GlobalC::ucell.omega
                        * GlobalC::ucell.lat0; // why?
